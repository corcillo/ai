...
class Product(list, Expression):
    ...
    def simplify(self):
        ...
        print self
        factors = []
        for factor in self:                 #iterate through terms in prod
            if isinstance(factor, Product): #if term is a product
                factors += list(factor)     #add that prods terms to terms
            else:
                factors.append(factor)      #otherwise, add the whole term
        result = Product([1])               #multiply all terms
        for factor in factors:
            result = multiply(result, simplify_if_possible(factor))#recursively simplify terms terms
        return result.flatten() #again, add all terms in prods to outer list
...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1,Product) and isinstance(expr2,Product):
        ...
        sumOfProducts = []
        for adder in expr2:
            sumOfProducts.append(Product(list(expr1)+[adder]))
        return Sum(sumOfProducts)
    
    elif isinstance(expr1,Sum) and isinstance(expr2,Product):
        sumOfProducts = []
        for adder in expr1:
            sumOfProducts.append(Product(list(expr2)+[adder]))
        return Sum(sumOfProducts)
    elif isinstance(expr1,Sum) and isinstance(expr2,Sum):
        sumOfProds = []
        for termL in expr1:
            for termR in expr2:
                sumOfProds.append(Product([termL,termR]))
        return Sum(sumOfProds)
